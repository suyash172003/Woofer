...
function Artists() {
  ...
  const fetchTrack=async()=>{
    const response=await fetch('https://v1.nocodeapi.com/dancho/spotify/duUezEoocvClHaZN/browse/categories')
    const data=await response.json()
    setTrack(data.categories.items)
                    ...
                    <div key={index} className='container  h-1/3 w-1/5'>
                        <div className='rounded-xl w-full  h-full container bg-fuchsia-500 flex flex-col justify-center items-center'>
                            <div className='h-full w-full flex flex-row justify-center'>
                                ...
                                <img src={element.icons[0].url} alt="Music Icon" className="ml-5 mt-3 rounded-xl h-1/2 w-1/3 z-10"/>
