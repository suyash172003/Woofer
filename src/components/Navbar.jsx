    ...
    <div className='w-full h-screen flex justify-between'>
      <div className="flex flex-col items-center justify-start navbar w-full h-full bg-slate-50 gap-2 leading-none">
         ...
         <div className="px-20 py-1 h-full w-full flex flex-col justify-center items-center gap-3">
            <div className='-text-xl font-semibold h-full w-full flex flex-col justify-center items-start text-black space-y-4'>{["Home","Mood","Albums","Artists"].map((items,index)=>(<motion.div whileHover={{scale:1.2}} key={index} className='flex items-center justify-between gap-4'><AiFillHeart className='text-fuchsia-500'></AiFillHeart><Link to={index===0 ?"/":`/${items.toLowerCase()}`} key={items}>{items}</Link></motion.div>))}</div>
         </div>
